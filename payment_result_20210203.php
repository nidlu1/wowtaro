...
$pa_point = trim($_REQUEST["MallReserved"]);

$pay_table_row = sql_fetch("SELECT * FROM ".$g5['pay_table']." WHERE pa_point='".$pa_point."'");
...
/* 
 *  결제할 때 포인트 사용시 받는 포인트값.
 */
$pointpay = $_REQUEST['pointpay'];
...
//od_receipt_time='".substr($MOID, 0, 4)."-".substr($MOID, 4, 2)."-".substr($MOID, 6, 2)." ".substr($MOID, 8, 2).":".substr($MOID, 10, 2).":".substr($MOID, 12, 2)."'
        ...
        /* 포인트로 결제할시 해당 값을 저장하는 로직1
         */
        if(isset($pointpay)){
            insert_point($member['mb_id'] , $pointpay*(-1) , $MOID.', '.$PayMethod.', '.$pointpay.' 포인트사용'   , "@usePoint", $member['mb_id'] , $MOID.', '.$PayMethod.', '.$pointpay.' 포인트사용' );
        /* 포인트로 결제할시 해당 값을 저장하는 로직 끝
         */
        }
	...
	if ($ResultCode == "3001") {
		include_once G5_PATH."/update_charge_human.php";
                //포인트 지급.
		insert_point($member['mb_id'],  $pa_point, $MOID.', '.$PayMethod.', '.$Amt.' 충전'  , '@charge', $member['mb_id'], $MOID.', '.$PayMethod.', '.$Amt.' 충전'); //insert_point 함수에도 060결제로직이 있어 중복결제 가능성때문에 주석처리.
?>
