...
<!-- 메인상담사진열 10 시작 { -->
<?php
$li_width = intval(100 / $item_mod);
$li_width_style = ' style="width:'.$li_width.'%;"';

for ($i=0; $row=sql_fetch_array($result); $i++) {

    if ($item_mod >= 2) { // 1줄 이미지 : 2개 이상
        if ($i%$item_mod == 0) $li_clear = 'sct_last'; // 줄 마지막
        else if ($i%$item_mod == 1) $li_clear = 'sct_clear'; // 줄 첫번째
        else $li_clear = '';
    } else { // 1줄 이미지 : 1개
        $li_clear = 'sct_clear';
    }
...
/*이미지영역 끝*/
    ...
    if ($row['mb_no']) {
        //echo "<div class=\"sct_id\">&lt;".stripslashes($row['mb_no'])."&gt;</div>\n";
    }
...
<!-- } 상담사진열 10 끝 -->
  ...
  <!--div class="main-more">
    <p>상담사 더 보기</p> <i class="xi-angle-down-min"></i>
  </div-->
<?php
