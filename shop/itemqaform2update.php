...
if (!$is_member) {
    alert_close("상담문의는 회원만 작성이 가능합니다.");
}
...
if ($w == "")
{
    ...
    $alert_msg = '상담문의가 등록 되었습니다.';
}
else if ($w == "u")
{
    if (!$is_admin)
    {
        $sql = " select count(*) as cnt from {$g5['g5_shop_item_qa_table']} where it_id = '{$member['mb_no']}' and iq_id = '$iq_id' ";
        $row = sql_fetch($sql);
        if (!$row['cnt'])
            alert("자신의 상담문의만 수정하실 수 있습니다.");
    }
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        ...
        $row = sql_fetch($sql);
        if (!$row)
            alert("자신의 상담문의만 삭제하실 수 있습니다.");

        if ($row['iq_answer'])
            alert("답변이 있는 상담문의는 삭제하실 수 없습니다.");
    }
