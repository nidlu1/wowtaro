...
function itemqa_page($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $url = preg_replace('#&amp;page=[0-9]*#', '', $url) . '&amp;pageqa=';
...
function itemqa_page_moblie($write_pages, $cur_page, $total_page, $url, $add="")
{
    //$url = preg_replace('#&amp;page=[0-9]*(&amp;page=)$#', '$1', $url);
    $url = preg_replace('#&amp;page=[0-9]*#', '', $url) . '&amp;pageqa=';
	$str = '';
...
if ($pageqa < 1) $pageqa = 1; // 페이지가 없으면 첫 페이지 (1 페이지)
$from_record = ($pageqa - 1) * $rows; // 시작 레코드 구함
