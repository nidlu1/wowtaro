...
if (G5_IS_MOBILE) {
    include_once(G5_MSHOP_PATH.'/itemuseform2.php');
    return;
}
...
if (!$is_member) {
	...
	echo "<script>alert('상담후기는 회원만 작성 가능합니다.');opener.location.href='".G5_BBS_URL."/login.php';self.close();</script>";
}
...
// 상품정보체크
$sql = " select mb_no from {$g5['member_table']} where mb_no = '$it_id' ";
$row = sql_fetch($sql);
if(!$row['mb_no'])
    alert_close('상담사회원이 존재하지 않습니다.');
...
if ($w == "") {
    ...
    if (!$is_admin && $it_id != $member['mb_no']) {
...
$editor_html = editor_html('is_reply_content', get_text(html_purifier($use['is_reply_content']), 0), $is_dhtml_editor);
$editor_js = '';
