...
function valid_mb_hp($reg_mb_hp)
{
    ...
    if(!$reg_mb_hp)
        return "인증확인 버튼을 눌러 휴대폰번호를 인증해 주십시오.";
    else {
...
{
    global $g5;
    $row = sql_fetch(" select count(*) as cnt from {$g5['member_table']} where mb_recommend = '$reg_mb_recommend' ");
    if ($row['cnt'])
        return "이미 추천인으로 등록된 아이디입니다.";
    else
        return "";
}
function exist_mb_recommend2($reg_mb_recommend)
{
    global $g5;
    $row = sql_fetch(" select count(*) as cnt from {$g5['shop_order_table']} where mb_id = '$reg_mb_recommend' and od_receipt_price > 30000;  ");
    if ($row['cnt'])
        return "결제한 회원만 추천인으로 등록할 수 있습니다.";
    else
        return "";
}
return;
