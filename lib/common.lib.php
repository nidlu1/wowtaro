...
function get_member($mb_id, $fields='*')
{
	...
	//$mb_id = ( !preg_match("/([0-9a-zA-Z_-]+)@([0-9a-zA-Z_-]+)\.([0-9a-zA-Z_-]+)/", $mb_id) && preg_match("/[^0-9a-z_]+/i", "", $mb_id) ) ? "" : $mb_id;
//echo "mb_id = ".$mb_id;
...
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0)
{
    ...
    if ($mb_id == '') { return 0; }
    $mb = sql_fetch(" select mb_id,mb_hp from {$g5['member_table']} where mb_id = '$mb_id' ");
    ...
    //od_id 빼오기
    $p_od_id =(int) substr($content, 0, 14);
    switch ($p_od_id){
        case 0:
            $p_od_id = 0;
            break;
...
function get_cate_list()
{
	...
	$qsql = "SELECT * FROM ".$g5['g5_shop_category_table']." WHERE ca_use='1' ORDER BY ca_order ASC";
	$qres = sql_query($qsql);
	$qdt = NULL;
...
function get_hash_list()
{
	...
	$qsql2 = "SELECT * FROM ".$g5['hashtag_table']." ORDER BY ht_no";
	$qres2 = sql_query($qsql2);
	$qdt2 = NULL;
	while ( $qrow2 = sql_fetch_array($qres2) ) {
		$qdt2[] = $qrow2;
	}
...
// 간단한 파일 업로드
function file_up_func ( $file, $image_regex, $upload_dir, $file_name ) {
	// 대표이미지 업로드
	if (isset($_FILES[$file]) && is_uploaded_file($_FILES[$file]['tmp_name'])) {
		if (!preg_match($image_regex, $_FILES[$file]['name'])) {
			alert($_FILES[$file]['name'] . '은(는) 허용된 파일형식이 아닙니다.');
		}
...
// 상담사 회원 상담분류 가져오기
function b_cat_func ( $cat ) {
	global $g5;

	$arr = explode(",", $cat);
	$cat_str = "'".implode("','", $arr)."'";
	$sql = "SELECT * FROM ".$g5['g5_shop_category_table']." WHERE ca_use='1' AND ca_id IN (".$cat_str.") ORDER BY ca_order ASC";
	$res = sql_query($sql);
	$dt = NULL;
...
// 상담사 회원 상담분류 가져오기
function s_cat_func ( $cat ) {
	global $g5;

	//$arr = explode(",", $cat);
	//$cat_str = "'".implode("','", $arr)."'";
	$sql = "SELECT * FROM ".$g5['hashtag_table']." WHERE ht_no IN (".$cat.") ORDER BY ht_no ASC";
	$res = sql_query($sql);
	$dt = NULL;
	while ( $row = sql_fetch_array($res) ) {
		$dt[] = $row;
...
// 2차원배열에서 값으로 key 반환, (비교활 인자 $k, 값 $id, 배열 $array)
function searchForId($k, $id, $array) {
	//echo $k." , ".$id;
...
function searchForId3($id, $array) {
   foreach ($array as $key => $val) {
       if ($val['ca_id'] === $id) {
           return $key;
       }
...
// 전문가 상태값 확인
function counsel_stat($cp_code) {
	global $cp, $counsel_stat_chk;
...
function counsel_stat_update() {
	global $g5, $cp, $counsel_stat_chk, $counsel_stat_chk2;
	...
	$url2 = $counsel_stat_chk2."?cp_code=".$cp;
...
*/
	...
	foreach ($arr as $key => $val) {
		...
		// 할인상담이 예약대기 인 경우 일반상담이 상담중이면 상담중 
