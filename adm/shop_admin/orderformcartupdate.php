...
if (in_array($_POST['ct_status'], $status_cancel)) {
    ...
    //if($row['od_count1'] == $row['od_count2']) {
        ...
        if($pg_cancel == 1) {
            ...
            if($od['od_tno'] && ($od['od_settle_case'] == 'CARD' || $od['od_settle_case'] == 'CARS' || $od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '간편결제' || $od['od_settle_case'] == 'KAKAOPAY') || ($od['od_pg'] == 'inicis' && is_inicis_order_pay($od['od_settle_case']) )) {
                switch($od['od_pg']) {
                    case 'lg':
				...
				if ( $od['od_pg'] == "innopay" ) {
					$json = "{\"mid\":\"".$MID."\",\"tid\":\"".$od['od_tno']."\",\"svcCd\":\"01\",\"partialCancelCode\":\"0\",\"cancelAmt\":\"".$od['od_receipt_price']."\",\"cancelMsg\":\"\",\"cancelPwd\":\"".$pg_cancel_pwd."\"}";
					$json_ret = Curl($pg_cancel_url, $json, $http_status);
					$json_arr = json_decode($json_ret);
					if ( $json_arr->resultCode != "2001" ) {
						//print_r($json_arr);exit;
						$pg_res_cd = $json_arr->resultCode;
                        $pg_res_msg = $json_arr->resultMsg;
						//alert( "'[".$json_arr->resultCode."] ".$json_arr->resultMsg."' 으로 취소가 실패하였습니다.", );
						//exit;
					}
				}
		...
		$tel = trim(str_replace("-","",$od2['od_hp']));
		$amt = $od2['od_cart_price'];
		$sec = $od2['od_pay_time'];
...
if (in_array($_POST['ct_status'], $status_cancel)) {
	$info['od_cancel_price'] = $info['od_cart_price'];
	$info['od_cart_price'] = $info['od_cart_price'] - $info['od_cancel_price'];
}
//print_r($info);exit;
if(!$info)
...
if ( $_POST['ct_status'] == "입금" && $od['od_settle_case'] == "무통장" ) {
	$qry = "SELECT COUNT(*) cnt FROM ".$g5['point_table']." WHERE mb_id='".$od['mb_id']."' AND po_rel_table='@charge' AND po_content LIKE '".$od_id."%'";
	$cnt_qry = sql_fetch($qry);
	if ( $cnt_qry['cnt'] <= 0 ) {
		$MOID = $od_id;
		$ORDER_ID = $MOID;
		$AUTH_AMOUNT = $od["od_cart_price"];
		$hp = $od['od_hp'];
