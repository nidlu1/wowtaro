...
$sql_common = "  from {$g5['g5_shop_item_use_table']} a
                 left join {$g5['member_table']} b on (a.it_id = b.mb_no)
...
$sql  = " SELECT a.*,c.*,b.mb_nick it_name, b.mb_no it_id
          $sql_common
...
<select name="sfl" id="sfl">
    <option value="b.mb_nick" <?php echo get_selected($sfl, 'b.mb_nick'); ?>>상품명</option>
    <option value="a.it_id" <?php echo get_selected($sfl, 'a.it_id'); ?>>상품코드</option>
    <option value="is_name" <?php echo get_selected($sfl, 'is_name'); ?>>이름</option>
    <option value="c.mb_hp" <?php echo get_selected($sfl, 'c.mb_hp'); ?>>핸드폰 번호</option>
</select>
...
<div class="tbl_head01 tbl_wrap" id="itemuselist">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $mb_hp = get_sideview($row['mb_id'], get_text($row['mb_hp']), $row['mb_email'], $row['mb_homepage']);
        $bg = 'bg'.($i%2);
    ?>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td style="width: 150px"><?php echo $mb_hp; ?></td>
        <td class="td_select">
		...
		<td class="td_chk2">
            <label for="best_<?php echo $i; ?>" class="sound_only">BEST</label>
