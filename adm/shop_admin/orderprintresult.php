...
if ($csv == 'csv')
{
	...
	$sql = " SELECT od_id, od_zip1, od_zip2, od_addr1, od_addr2, od_addr3, od_addr_jibeon, od_name, od_tel, od_hp, od_memo, od_invoice, od_status
               FROM {$g5['g5_shop_order_table']}
              where 1 ";
    if ($case == 1) // 출력기간
        $sql .= " and od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
    else // 주문번호구간
        $sql .= " and od_id between '$fr_od_id' and '$to_od_id' ";
    if ($ct_status)
        $sql .= " and od_status = '$ct_status' ";
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        echo '"'.print_address($row['od_addr1'], $row['od_addr2'], $row['od_addr3'], $row['od_addr_jibeon']).'"'.',';
        echo '"'.$row['od_name'].'"'.',';
        //echo '"'.multibyte_digit((string)$row[od_tel]).'"'.',';
        //echo '"'.multibyte_digit((string)$row[od_hp]).'"'.',';
...
if ($csv == 'xls')
{
    ...
    for($i=1; $row=sql_fetch_array($result); $i++)
    {
        ...
        $worksheet->write($i, 1, print_address($row['od_addr1'], $row['od_addr2'], $row['od_addr3'], $row['od_addr_jibeon']));
...
if ($case == 1)
{
    $fr_date = date_conv($fr_date);
    $to_date = date_conv($to_date);
    $sql = " SELECT a.od_id FROM {$g5['g5_shop_order_table']} a
              where a.od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
}
else
{
    $sql = " SELECT a.od_id FROM {$g5['g5_shop_order_table']} a
              where a.od_id between '$fr_od_id' and '$to_od_id' ";
}
if ($ct_status)
    $sql .= " and a.od_status = '$ct_status' ";
