	...
	<form method="post">
		...
		<tr>
			...
			<td>
			    <?php
				$count = 1;
				$qry = sql_query("select * from {$g5['sms5_form_table']} where fg_no='1' order by fo_no desc");
		...
		echo "<tr><td colspan='2' align='center'><br><br>전송이 모두 완료되었습니다. <input type='button' value='확 인' onclick='location.replace(\"sms_send2.php\");' style='cursor:pointer;padding:7px;background-color:red;color:#ffffff;border:1px solid red;'><br><br></td></tr>";
		...
		<tr>
			...
			$sql = "select * from g5_member where mb_id NOT LIKE '%@wowunse.com%' and mb_level = '2' GROUP BY mb_hp order by mb_name asc ";
			...
			<td>
			<br>
			<font style='color:red;font-size:10pt;font-weight:bold'>(상담사 제외/핸드폰 미등록 제외 / 중복번호 제거)</font>
			<br><br>
			<select name = "sms_to[]" class="frm_input" multiple style="height:300px;" >
			<?php while($rs = sql_fetch_array($stmt)){
				$rs['mb_hp']=preg_replace('/[^0-9]/','',$rs['mb_hp']);
				if($rs['mb_hp']!=''){
					echo "<option value=".$rs['mb_hp'].">"; echo $rs['mb_hp']." [".$rs['mb_name']."]"; 
				}
			}?>
			</select>
			</td>
		...
		<tr>
			<th style="line-height:200%;">수기번호<div>
			<td>
				<input type='text' name='sms_to[]'>
			</td>
		</tr>
		<tr>
			<th>보내는 사람 번호</th>
