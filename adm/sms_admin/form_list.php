...
function multi_update(sel)
{
    ...
    if (sel.value == 'del') {
        if (!confirm("선택한 문자내용을 삭제합니다.\n\n비회원만 삭제됩니다.\n\n회원을 삭제하려면 회원관리 메뉴를 이용해주세요.\n\n실행하시겠습니까?"))
        {
            sel.selectedIndex = 0;
            return;
        }
    } else if (!confirm("선택한 문자내용을 " + sel.options[sel.selectedIndex].innerHTML + "\n\n실행하시겠습니까?")) {
        sel.selectedIndex = 0;
        return;
    }
...
<div class="local_sch01 local_sch sms_preset_sch">
    ...
    <select name="st" id="st">
        ...
        <option value="content"<?php echo get_selected('content', $st); ?>>내용</option>
    </select>
...
<div class="btn_fixed_top" >
    ...
    <a href="./form_write.php?page=<?php echo $page?>&amp;fg_no=<?php echo $fg_no?>" class="btn btn_01">문자내용 추가</a>
</div>
