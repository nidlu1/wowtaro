...
if ($sca != "") {
    $sql_search .= " AND mb_1 like '%".$sca."%' ";
}

if ($gubun != "") {
    $sql_search .= " AND mb_gubun = '".$gubun."' ";
	$qstr .= "&gubun=".$gubun;
}
...
<label for="gubun" class="sound_only">회원유형</label>
<select name="gubun" id="gubun">
    <option value="" <?php echo get_selected($_GET['gubun'], ""); ?>>전체상담사회원</option>
    <option value="1"<?php echo get_selected($_GET['gubun'], "1"); ?>>일반상담사회원</option>
    <option value="2"<?php echo get_selected($_GET['gubun'], "2"); ?>>가족상담사회원</option>
</select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" rowspan="2" id="mb_list_cert"><?php echo subject_sort_link('mb_1', '', 'desc') ?>상담분류</a></th>
        <th scope="col" id="mb_list_mailc"><?php echo subject_sort_link('mb_email_certify', '', 'desc') ?>메일인증</a></th>
		<th scope="col" id="mb_list_mng">회원유형</th>
		...
		<th scope="col" rowspan="2" id="mb_list_mng">상태고정</th>
    ...
    <tr>
		...
		<th scope="col" id="mb_list_cert">계약동의</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if ($is_admin == 'group') {
            ...
            $s_mod = '<a href="./member_form2.php?'.$qstr.'&amp;w=u&amp;mb_id='.$row['mb_id'].'" class="btn btn_03">수정</a>';
        }
		...
		$mb_1_arr = explode(",",$row['mb_1']);
		$mb_1_str = "";
		/*for ($ii = 0; $ii < count($mb_1_arr); $ii++) {
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td headers="mb_list_open">
            <?php echo $row['mb_gubun'] == 2 ? "상담사 가족" : "상담사 일반"; ?>
        </td>
        <!--td headers="mb_list_open">
		...
		<td headers="mb_list_mng" rowspan="2" class="td_mng td_mng_s"><input type="checkbox" name="mb_lock[<?php echo $i; ?>]" <?php echo $row['mb_lock']?'checked':''; ?> value="1" id="mb_lock_<?php echo $i; ?>"></td>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="mb_list_cert">
            <label for="mb_cert_<?php echo $i; ?>" class="sound_only">계약동의</label>
            <?php echo $row['mb_cert'] == "0000-00-00 00:00:00"?'동의안함':$row['mb_cert']; ?>
        </td>
