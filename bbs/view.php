...
if ($sca || $stx || $stx === '0') {
    ...
    $search_href = $bbs_dir.'board.php?bo_table='.$bo_table.'&amp;page='.$page.$qstr;
    $list_href = $bbs_dir.'board.php?bo_table='.$bo_table;
} else {
    $search_href = '';
    $list_href = $bbs_dir.'board.php?bo_table='.$bo_table.'&amp;page='.$page;
}
...
if (isset($prev['wr_id']) && $prev['wr_id']) {
    ...
    $prev_href = $bbs_dir.'board.php?bo_table='.$bo_table.'&amp;wr_id='.$prev['wr_id'].$qstr;
...
if (isset($next['wr_id']) && $next['wr_id']) {
    ...
    $next_href = $bbs_dir.'board.php?bo_table='.$bo_table.'&amp;wr_id='.$next['wr_id'].$qstr;
...
if ($member['mb_level'] >= $board['bo_write_level'])
    $write_href = $bbs_dir.'write.php?bo_table='.$bo_table;

// 답변 링크
$reply_href = '';
if ($member['mb_level'] >= $board['bo_reply_level'])
    $reply_href = $bbs_dir.'write.php?w=r&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr;

// 수정, 삭제 링크
$update_href = $delete_href = '';
// 로그인중이고 자신의 글이라면 또는 관리자라면 비밀번호를 묻지 않고 바로 수정, 삭제 가능
if (($member['mb_id'] && ($member['mb_id'] === $write['mb_id'])) || $is_admin) {
    $update_href = $bbs_dir.'write.php?w=u&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    set_session('ss_delete_token', $token = uniqid(time()));
    $delete_href =$bbs_dir.'delete.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;token='.$token.'&amp;page='.$page.urldecode($qstr);
}
else if (!$write['mb_id']) { // 회원이 쓴 글이 아니라면
    $update_href = $bbs_dir.'password.php?w=u&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    $delete_href = $bbs_dir.'password.php?w=d&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
}

// 최고, 그룹관리자라면 글 복사, 이동 가능
$copy_href = $move_href = '';
if ($write['wr_reply'] == '' && ($is_admin == 'super' || $is_admin == 'group')) {
    $copy_href = $bbs_dir.'move.php?sw=copy&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    $move_href = $bbs_dir.'move.php?sw=move&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
}
...
if ($is_member) {
    // 스크랩 링크
    $scrap_href = $bbs_dir.'scrap_popin.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id;

    // 추천 링크
    if ($board['bo_use_good'])
        $good_href = $bbs_dir.'good.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;good=good';

    // 비추천 링크
    if ($board['bo_use_nogood'])
        $nogood_href = $bbs_dir.'good.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;good=nogood';
}
