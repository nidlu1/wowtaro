...
if ($w == '' || $w == 'u') {
    ...
    if ($w=='') {
        ...
        if (get_session('ss_check_mb_id') != $mb_id || get_session('ss_check_mb_nick') != $mb_nick) {	// 아이디를 이메일형식으로 사용하므로 이메일 체크 필요없음 || get_session('ss_check_mb_email') != $mb_email) {
...
//                                sql_query($sql_use_list);
	...
	if (get_encrypt_string($mb_password_origin) != $member['mb_password']) {
        echo '
        <!doctype html>
        <html lang="ko">
        <head>
