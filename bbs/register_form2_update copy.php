...
$mb_hashtag     = isset($_POST['mb_hashtag'])       ? trim($_POST['mb_hashtag'])     : "";
...
if ($w == '' || $w == 'u') {
    ...
    if ($w=='') {
        ...
        if (get_session('ss_check_mb_email') != $mb_email || get_session('ss_check_mb_nick') != $mb_nick) {	// 아이디를 이메일형식으로 사용하므로 이메일 체크 필요없음 || get_session('ss_check_mb_id') != $mb_id) {
...
if ($w == '') {
	...
	if ( count($sub_sql) > 0 ) {
		$sub_sql_str = implode(" , ", $sub_sql);
		$sql = "UPDATE ".$g5['member_table']." SET ".$sub_sql_str." WHERE mb_no='".$mb_no."'";
		sql_query($sql);
	}
...
if ($w == '') {
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        <title>회원정보수정</title>
        <body>
        <form name="fregisterupdate" method="post" action="'.G5_HTTP_BBS_URL.'/register_form2.php">
